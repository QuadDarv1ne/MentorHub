...
export default function RegisterPage() {
  ...
  // We don't need the login function here since we're redirecting to login page after registration
  useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // In a real implementation, you might want to redirect to login page instead
  ...
  const handleSocialRegister = async (provider: 'google' | 'github') => {
    ...
    setTimeout(() => {
      ...
      setError(`Регистрация через ${provider === 'google' ? 'Google' : 'GitHub'} пока не реализована`)
    }, 1500)
  }

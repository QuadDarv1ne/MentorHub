...
async def create_payment_intent(
    ...
    if "error" in intent_result:
        raise HTTPException(
            ...
            detail=f"Ошибка создания платежа: {intent_result['error']}"
        )
...
async def refund_payment(
    ...
    if payment.transaction_id:
        ...
        if "error" in refund_result:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=f"Ошибка возврата: {refund_result['error']}"
            )
...
async def get_payment_history(
    ...
    payments = db.query(DBPayment).filter(
        DBPayment.student_id == current_user.id
    ).order_by(DBPayment.created_at.desc()).offset(skip).limit(limit).all()

...
async def create_mentor(mentor: MentorCreate, db: Session = Depends(get_db)):
...
async def update_mentor(mentor_id: int, mentor: MentorUpdate, db: Session = Depends(get_db)):
...
async def delete_mentor(mentor_id: int, db: Session = Depends(get_db)):

...
class NotificationService:
    ...
    async def send_notification(
        self, user_email: str, notification_type: str, data: Dict[str, Any], channels: list = None
    ) -> bool:
        """
        ...
        if "email" in channels:
            success = await self._send_email_notification(user_email, notification_type, data)

        # Future: Add more channels (SMS, Push, In-App)

        return success

    async def _send_email_notification(self, email: str, notification_type: str, data: Dict[str, Any]) -> bool:
    ...
    def _handle_welcome(self, email: str, data: Dict[str, Any]) -> bool:
        ...
        return self.email_service.send_welcome_email(email, data.get("name", "User"))
    ...
    def _handle_session_confirmed(self, email: str, data: Dict[str, Any]) -> bool:
        ...
        return self.email_service.send_session_confirmation(
            email, data.get("topic", "Mentoring Session"), data.get("scheduled_time", "")
        )

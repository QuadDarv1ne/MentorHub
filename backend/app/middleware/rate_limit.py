...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, max_requests: int = 100, time_window: int = 60, exclude_paths: list = None):  # seconds
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        response.headers["X-RateLimit-Remaining"] = str(max(0, self.max_requests - bucket.get("count", 0)))
        response.headers["X-RateLimit-Reset"] = str(int(bucket.get("reset_time", datetime.now()).timestamp()))
